...
![image](https://github.com/user-attachments/assets/0430891f-815a-4962-9e59-f2d04553c3ca)

해당 예시 코드를 보면, 쉽게 이해할 수 있다. 상속과 관련된 아주 유명한 예시 코드이다.

Animal이라는 클래스를 **Dog** 클래스와 **Cat** 클래스가 상속받고 있다. 이러면, 부모 클래스에서 구현한 info라는 메서드를 굳이 선언하지 않아도 사용할 수 있고, 추상화할 때 더 알아보겠지만, sound라는 추상 메서드를 오버라이드하여 자식 클래스에 맞게 구현할 수도 있다.

이렇게 하면, 공통적인 함수는 굳이 다시 작성하지 않아도 되고, 만약 하나를 수정해야 하는 일이 벌어지더라도, 모든 클래스에서 일일이 수정할 필요가 없이, 부모 클래스의 공통되는 메서드만 하나 수정하면 된다. 이로써 유지보수성을 높일 수 있고, 더 객체지향적인 클래스의 모습을 갖출 수 있게 되었다.

이렇게 클래스 간의 공통적인 메서드를 공유해야 할 경우나, 명확한 계층 구조가 필요한 경우에 코드의 재사용성과 유지보수성을 높일 수 있도록 하는 객체지향의 특징이 바로 **상속**이다.

나는 상속을 공부하면서 하나 의구심이 들었던 것이, 그러면 **무조건 상속을 많이 하는 게 좋은 걸까?** 라는 의문이었다. 캡슐화나, 앞으로 알아볼 다형성에 대해서는 크리티컬한 단점이 보이지 않지만, **상속**의 경우 무분별하게 사용하게 될 시, 크리티컬한 문제가 생길 수 있다는 것을 알게 되었다.

첫 번째는, 상속에 대하여 명확히 알지 못한 상태로 무분별하게 사용한다면, 캡슐화를 깨고 오히려 **객체지향적이지 못한 코드를 만들어낸다는 것**이다.

![image](https://github.com/user-attachments/assets/92f01f8d-d8ca-445d-93f4-a136a689ac0f)

예를 들어 이렇게 Collection의 HashSet을 상속받아 만든 DidimHashSet이라는 새로운 자료구조가 있다. 이 자료구조는 HashSet의 기능을 그대로 사용하면서 동시에 인스턴스를 생성한 이후에 들어오는 원소의 숫자를 카운팅하려고 만든 자료구조 형태이다.

이 코드가 과연 문제 없이 안전하게 동작할까?

**정답은 X 이다.**

![image](https://github.com/user-attachments/assets/2cd3858f-10d4-4a06-b4e8-180c42013282)

이 코드의 부모 클래스인 AbstractCollection과 HashSet을 보면, addAll이라는 메서드 안에서 add라는 메서드를 불러오고 있다.

![image](https://github.com/user-attachments/assets/f0fd887f-c707-4098-aaf3-31ab9f9fbf5c)

부모의 `addAll`을 호출하면, **AbstractCollection.java**에 구현되어 있는 `addAll`을 그대로 사용한다.

이 내부에서 `add` 메서드를 호출하게 된다. **자바의 셀프 참조성**에 의해 DidimHashSet부터 오버라이딩된 `add` 메서드가 있는지 탐색한다.

마침 DidimHashSet.add에서 오버라이딩이 되어있다.

그렇기 때문에, DidimHashSet.add가 총 3번 호출되어 카운팅이 3번 더 올라가게 된다.

따라서 문제가 발생하는 코드를 입력했을 때, 원래 기대한 값은 3이 카운팅되어야 하지만, **총 6이 카운팅되는 모습을 볼 수 있다.**

이처럼 상속을 사용한 후, 메서드를 재정의한 경우에 문제가 생길 수 있다.

이러한 문제를 해결하려면, 부모 클래스가 어떻게 구성되어 있는지 알아야 한다는 것이 기저에 깔려있어야 한다.

즉 내부 구현을 불필요하게 노출하는 셈이 되는 것이다. 이는 위에서 이야기한 **캡슐화를 파괴하는 코드**가 된다.

만약 addAll 메서드가 add를 호출하는 것을 알고, DidimHashSet에서 `addAll` 메서드를 오버라이딩 하지 않았다고 하더라도 지금 당장 프로그램은 정상적으로 돌아가지만, 다음 릴리즈에서도 이럴 것이라는 보장 역시 없다.

그 외에도 너무 많은 상속이 있다면, 부모 클래스의 필드와 메서드를 찾는 데에 불편함이 생기며, 부모 클래스의 결함을 자식 클래스가 그대로 떠안는 문제가 생기는 등 예측하기 어려운 문제가 생길 수 있다.

그렇기에 이펙티브 자바에서는 상속이 적절한 경우는 클래스의 행동을 **확장(extend)**하는 것이 아니라 **정제(refine)**할 때라고 한다. 여기서 확장이란 새로운 행동을 덧붙여 기존의 행동을 부분적으로 보완하는 것을 의미하고, 정제란 **부분적으로 불완전한 행동을 완전하게 만드는 것**을 의미한다. 심지어 “**무분별한 상속은 객체지향을 망가뜨린다**"라며, 아예 상속의 사용을 지양해야 한다는 글도 있다.

개발을 하다 보면 많이 접하는 단어 중 **트레이드 오프**라는 단어가 있다. 어떠한 경우에 상속을 사용하고, 상속을 많이 받을 경우 어떠한 문제가 생기는 등에 대해 명확히 알고 적절히 사용한다면, 상속은 분명 코드의 재사용성을 높여줄 수 있는 자바의 강력한 무기가 될 것이다.
