...
첫 번째로, `GenericAppenderHelper.java` 코드에서, 생성자가 @Deprecated 되어있었습니다. 과거에 JUnit에서 흔히 쓰던 TemporaryFolder 객체를 인자로 받습니다. TemporaryFolder는 테스트 중 임시 파일/디렉토리를 자동 생성하고 정리해주는 도구입니다. 아마, Junit4 의존성을 완전히 걷어내기 위한 과정에서 @Deprecated 를 해둔 거 같습니다. 하지만 위에서 이야기하였듯 현재 완전히 걷어내는 것은 어렵다고 판단하였기 때문에, @Deprecated 를 삭제해주었습니다. ( 삭제하지 않으면, 빌드가 되지 않습니다 .)
