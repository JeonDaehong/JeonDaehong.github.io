...
위 그림은 **Erasure Coding가 적용되기 전의 Hadoop 블록 구조를 기반으로 패리티를 구성한 방식**과, **stripping 이후 패리티를 구성한 방식의 차이**를 나타낸 것입니다. **Hadoop의 논리적인 블록 사이즈는 여전히 동일**하게 유지됩니다.

**Contiguous block 구조**는 **논리적 블록을 실제 스토리지 블록에 1:1로 매핑**하는 방식으로, 하나의 파일을 읽을 때 **블록 단위로 순차적인 접근이 가능**합니다.

반면, **striped block 구조**는 **논리적 블록을 더 작은 단위인 셀(cell)로 분할**합니다. **쓰기(write)** 작업은 셀들로 구성된 stripe들을 여러 개의 **storage block 세트에 round-robin 방식으로 저장**하고, **읽기(read)** 작업은 **여러 storage block 세트에서 병렬로 cell stripe를 읽어들입니다**.

이러한 방식 덕분에, **논리적인 블록 1개에 대해서도 병렬 처리가 가능해집니다**.
