...
클라이언트로부터 요청이 도착하면, **Reader Thread**가 이 요청을 **하나의 FIFO 큐**에 저장합니다. Handler가 작업을 수행할 준비가 되면, 이 큐에서 **맨 앞에 있는 요청을 하나씩 꺼내어 처리**합니다.

이 방식은 단순하고 구현이 쉬우며, 요청 순서를 그대로 유지할 수 있다는 장점이 있습니다.  
