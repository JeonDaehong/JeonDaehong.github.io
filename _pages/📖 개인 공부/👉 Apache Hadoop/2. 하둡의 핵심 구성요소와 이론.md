...
<img src="/assets/img/namenode.png" alt="namenode" />
...
### ✅ Read-Write 의 일관성 유지 방법
기존 하둡은 쓰기 작업이 완전히 종료되어 NameNode의 edit log에 반영되고, 해당 block이 최종적으로 commit되기 전까지는, 다른 클라이언트는 물론, 동일 클라이언트라도 그 데이터는 보장되지 않습니다. 즉, **부분적으로 쓴 데이터는 보이지 않고, 이전 상태만 보이게 됩니다.** 이는 HDFS가 strong consistency가 아니라, ***단일 writer-consistent***한 모델을 따르기 때문입니다.
그리고, 아직 파일이 close되지 않은 상태에서 다른 클라이언트가 읽으려 하면 HDFS는 해당 파일을 "in-progress" 상태로 간주하고, 경우에 따라 "파일 없음" 또는 "파일 열림 중" 오류를 발생시킬 수 있습니다.
어쨋든, 나름의 일관성을 유지하고 있었지만, Observer NameNode 가 도입되면서, Active NameNode는 쓰기 작업을 바로 알고 있지만 Observer는 edit log를 따라잡는 데 약간의 지연이 있기 때문에, 클라이언트가 "내가 방금 쓴 것"을 Observer에 요청하면 아직 반영이 안되는 문제가 발생한 것입니다.
그래서 도입된 것이 `State ID` 입니다.
