...
<br>
### 🐘 2.1.1. RM ↔ Client Interface Component

🔸 **ClientService**
ClientService는 사용자가 ResourceManager에게 요청을 보낼 수 있도록 해주는 공식 통로입니다. 예를 들어, 사용자가 hadoop jar 명령어로 애플리케이션을 제출하면, 이 요청은 ClientService가 받아서 처리하게 됩니다. 이 컴포넌트는 다음과 같은 요청들을 담당합니다:

- 애플리케이션 제출 (application submission)

- 애플리케이션 종료 요청

- 현재 클러스터 상태나 큐(queue) 정보 조회

이 서비스는 일반 사용자가 리소스를 요청하거나 결과를 확인할 수 있도록 API 게이트웨이 같은 역할을 수행합니다.

🔸 **AdminService**
관리자(Admin)의 요청은 일반 사용자 요청보다 우선되어야 하기 때문에, YARN은 AdminService를 따로 분리해 제공합니다. 예를 들어, 관리자가 클러스터의 노드 리스트를 리프레시하거나, 큐 설정을 변경하려 할 때 AdminService를 통해 요청하게 됩니다. 이는 root 권한을 가진 사용자가 시스템 설정을 직접 수정하는 것과 유사하며, 보안과 안정성 확보를 위한 설계입니다.

<br>
### 🐘 2.1.2. RM ↔ NM Interface Component

🔸 **ResourceTrackerService**
ResourceTrackerService는 각 NodeManager에서 주기적으로 보내는 heartbeat를 수신하고, 이 정보를 기반으로 클러스터 전체의 자원 상태를 파악합니다. 예를 들어, 어떤 노드에서 CPU가 거의 다 찼다면 이 정보가 스케줄러로 전달되어, 더 이상 해당 노드에 컨테이너가 할당되지 않도록 조정됩니다.

🔸 **NMLivelinessMonitor**
이 컴포넌트는 모든 노드의 **생존 여부(liveness)**를 감시합니다. 각 노드가 10분 동안 heartbeat를 보내지 않으면, 해당 노드는 dead 상태로 간주되고 ResourceManager는 그 노드에 있던 컨테이너들을 자동으로 회수하거나 다른 노드에 재할당하게 됩니다.

예를 들어, 특정 서버가 장애로 인해 전원이 꺼졌을 경우, 이 컴포넌트는 해당 노드의 상태를 DEAD로 바꾸고, ResourceManager가 다른 노드에서 작업을 재시작하도록 지시합니다.

🔸 **NodesListManager**
NodesListManager는 클러스터에 속할 수 있는 노드 목록을 초기 설정 파일로부터 불러오고, 유효한 노드만을 클러스터에 포함시킵니다. decommissioned된 노드도 계속 추적하여 잘못된 자원 할당을 방지합니다. 예를 들어, 보안상 문제가 발생한 노드를 제외 리스트에 넣으면, 이 매니저가 해당 노드를 클러스터에서 제거하는 역할을 합니다.
...
# 🐘 3. YARN Capacity scheduler 와 Queue
