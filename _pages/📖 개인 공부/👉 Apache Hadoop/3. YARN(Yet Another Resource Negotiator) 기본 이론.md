...
1. 클라이언트가 YARN에 작업을 제출하면 Resource Manager(RM)는 클러스터의 전체 자원 상황과 스케줄링 정책을 고려해 Application Master(AM)를 실행할 자원을 할당할 준비를 시작합니다.
2. Resource Manager는 작업 실행을 위해 Application Master가 시작될 컨테이너를 할당할 노드를 선정하고, 해당 노드의 Node Manager(NM)에 컨테이너 실행 요청을 보냅니다.
3. Node Manager는 요청받은 자원 상황에 맞춰 컨테이너를 생성하고, Application Master 프로세스를 시작한 뒤 그 결과를 Resource Manager에 보고합니다.
4. Application Master가 실행되면, 작업을 진행하기 위해 추가 컨테이너 자원을 Resource Manager에 요청하며, Resource Manager와 Node Manager는 협력해 필요한 작업 컨테이너를 할당하고 실행합니다.
5. 작업이 실행되는 동안 Node Manager는 컨테이너 상태와 노드 상태를 주기적으로 Resource Manager에 보고하며, Resource Manager는 이를 바탕으로 자원 재조정과 장애 대응을 수행합니다.
6. Application Master는 작업의 진행 상태를 관리하고, 작업 실패나 장애 발생 시 재시작 또는 복구 요청을 Resource Manager에 전달하여 안정적인 작업 수행을 보장합니다.
7. Resource Manager는 Node Manager로부터 주기적인 하트비트를 받아 클러스터 상태를 모니터링하며, 스케줄러가 정책에 따라 자원 배분을 최적화합니다.
8. 작업이 완료되면 Application Master는 Resource Manager에 작업 종료를 알리고, Resource Manager는 할당된 자원을 해제하며 Node Manager에 컨테이너 종료를 요청하여 작업을 정리합니다.
