...
---
YARN Timeline Service는 YARN 클러스터에서 실행되는 애플리케이션들의 상태와 이벤트, 그리고 다양한 메타데이터를 수집하고 저장하는 핵심 컴포넌트입니다.  
이 서비스는 단순한 작업 상태 정보뿐 아니라, 태스크 실행 현황, 실패 기록, 자원 사용 통계 등 세부적인 데이터를 한 곳에 모아 관리함으로써, 클러스터 운영자나 개발자가 애플리케이션 실행 현황을 쉽게 모니터링하고 문제를 분석할 수 있도록 지원합니다.  

Timeline Service는 YARN이 여러 애플리케이션을 동시에 관리하는 복잡한 환경에서, 각 애플리케이션이 남기는 방대한 이벤트 데이터를 효율적으로 처리하기 위해 설계되었습니다.  
예를 들어, MapReduce 작업의 각 태스크가 언제 시작되고 종료되었는지, 실행 도중 어떤 오류가 발생했는지, 또는 Spark 애플리케이션에서 컨테이너가 할당된 시점과 자원 사용량이 어떠했는지 같은 상세 정보를 Timeline Service가 수집하여 중앙 저장소에 기록합니다.  

이 서비스는 크게 두 가지 주요 버전으로 나눌 수 있는데, 첫 번째 버전인 Timeline Service v1은 단일 서버 구조로 동작하며 이벤트 데이터를 JSON 형태로 HDFS에 저장하는 방식을 사용했습니다.  
하지만 이 방식은 대규모 클러스터에서 확장성 및 성능 측면에서 한계가 있었기에, 이후 Timeline Service v2가 도입되어 분산 환경에서 여러 서버가 이벤트를 병렬 처리하고 저장할 수 있도록 개선되었습니다.  
v2에서는 주로 HBase 같은 NoSQL 데이터베이스를 백엔드 저장소로 활용하여 실시간 조회와 복잡한 쿼리를 지원하고, 여러 애플리케이션 프레임워크를 폭넓게 지원할 수 있도록 진화했습니다.  

Timeline Service의 동작은 다음과 같이 이해할 수 있습니다.  
애플리케이션이 실행되면서, 각 태스크나 컨테이너에서 발생하는 상태 변경, 자원 할당, 에러 발생 등의 이벤트가 Timeline Server로 전달됩니다.  
Timeline Server는 이 이벤트를 받아 저장소에 기록하고, 사용자는 웹 UI 또는 REST API를 통해 원하는 시점의 이벤트를 조회하거나 특정 조건으로 필터링해 분석할 수 있습니다.  
예를 들어, 클러스터 운영자는 "지난 주간 MapReduce 작업 중 실패한 태스크들의 원인은 무엇인가?"라는 질문을 Timeline Service를 통해 쉽고 빠르게 답을 얻을 수 있습니다.  

아래는 Timeline Service의 주요 구성 요소를 간략히 정리한 표입니다.

| 구성 요소          | 역할                                                         |
|------------------|------------------------------------------------------------|
| Timeline Server  | 이벤트 수집, 처리 및 저장 담당                                |
| Storage Backend  | 이벤트 데이터를 저장하는 시스템 (HDFS, HBase 등)             |
| Client API       | 애플리케이션과 ResourceManager가 Timeline Server와 통신하기 위한 REST API 제공 |
| UI / Dashboard   | 저장된 이벤트 데이터 시각화 및 모니터링 도구                   |

Timeline Service를 활성화하려면 YARN 설정 파일(yarn-site.xml)에 다음과 같은 속성을 추가해야 합니다.  
...
이 설정은 Timeline Service를 활성화하고, v2 버전을 사용하며 저장소로 HBase를 사용한다는 의미입니다.
만약 HBase 대신 HDFS를 사용할 수도 있으나, 대규모 데이터와 빠른 조회가 필요한 환경에서는 HBase가 더 적합합니다.

실제 활용 예시를 들어보면, 예를 들어 여러분이 클러스터에서 여러 Spark 애플리케이션을 실행하고 있다고 가정해보겠습니다.
각 애플리케이션은 여러 컨테이너를 할당받아 작업을 수행하는데, 중간에 일부 태스크가 실패하거나 자원 부족 문제가 발생할 수 있습니다.
Timeline Service는 이러한 실패 이벤트와 컨테이너 할당 로그, 자원 사용 통계를 모두 수집하여 저장합니다.
운영자는 Timeline Server의 UI를 통해 특정 애플리케이션을 선택하고, 실패한 태스크의 상세 로그와 자원 할당 내역을 한눈에 파악할 수 있어, 문제의 원인을 빠르게 진단하고 대응할 수 있습니다.

또한 Timeline Service는 단순히 과거 데이터를 저장하는 역할뿐만 아니라, 실시간 이벤트 처리도 지원합니다.
예를 들어, 대규모 배치 작업이 진행되는 동안, 운영자는 Timeline Service에서 제공하는 실시간 모니터링 정보를 활용해 진행 상태를 점검하고, 필요시 자원 재조정이나 작업 중단을 결정할 수 있습니다.

마지막으로, Timeline Service는 보안과 멀티테넌시 기능도 지원합니다.
여러 사용자가 동일 클러스터를 공유하는 환경에서 각 사용자별 또는 애플리케이션별 접근 권한을 세밀하게 관리할 수 있도록 하여, 민감한 작업 정보가 무분별하게 노출되지 않도록 보호합니다.
또한, 저장된 이벤트 데이터는 TTL(Time To Live) 정책에 따라 자동으로 삭제되도록 설정할 수 있어, 저장 공간을 효율적으로 관리할 수 있습니다.

이처럼 YARN Timeline Service는 클러스터 운영의 핵심 인프라로 자리잡고 있으며, 애플리케이션 실행 상태를 상세하게 기록하고 분석할 수 있도록 도와주는 매우 중요한 구성요소입니다.

이 부분을 공부하면서 느낀게, 직접 YARN 환경에서 Timeline Service를 활성화하고 다양한 애플리케이션을 모니터링해 보면, 장애 대응과 성능 최적화에 큰 도움을 받으실 수 있을 것이라고 생각합니다.
